...
function inferProvider(code: string): string {
...
const ONEFORALL_FALLBACK: NormalizedModel[] = [
  {
...
const STRAICO_FALLBACK: NormalizedModel[] = [
  {
...
async function fetchOneForAllModels(
  ...
  return models
    ...
    .map((m) => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const additionalFields: any[] = Array.isArray(m.additional_fields)
        ? m.additional_fields
        : [];
      const maxTokensField = additionalFields.find(
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (f: any) => f.code === "max_tokens" || f.code === "max_output_tokens",
      );
...
async function fetchStraicoModels(apiKey: string): Promise<NormalizedModel[]> {
  ...
  return chatModels.map((m) => ({
    ...
    ...(m.max_output
      ? { maxTokens: { min: 1, max: Number(m.max_output) } }
      : {}),
    ...(m.word_limit != null ? { wordLimit: Number(m.word_limit) } : {}),
    ...(m.pricing ? { pricing: m.pricing } : {}),
    ...(m.owned_by ? { provider: String(m.owned_by) } : {}),
    ...(m.model_type ? { modelType: String(m.model_type) } : {}),
    ...
    ...(m.metadata?.applications?.length
      ...
      : {}),
    ...(m.metadata?.features?.length ? { features: m.metadata.features } : {}),
    ...(m.metadata?.pros?.length ? { pros: m.metadata.pros } : {}),
    ...(m.metadata?.cons?.length ? { cons: m.metadata.cons } : {}),
    ...(m.metadata?.icon ? { icon: String(m.metadata.icon) } : {}),
...
export async function GET(req: NextRequest) {
  ...
  if (!provider || (provider !== "1forall" && provider !== "straico")) {
    return NextResponse.json(
      {
        error:
          "Invalid or missing provider. Use ?provider=1forall or ?provider=straico",
      },
      { status: 400 },
    );
  }
  ...
  const apiKey =
    provider === "1forall"
      ? req.headers.get("x-oneforall-key")
      : req.headers.get("x-straico-key");
