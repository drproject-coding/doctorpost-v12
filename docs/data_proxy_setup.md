...
| ------------------ | ------------- | ------------------- | ------------------- | ------------------------------ |
| _(not in config)_  | Yes           | Own only            | Own only            | `WHERE user_id = session_user` |
...
| ------------------------- | --------------------- | ----------------------- | ------------------------- |
| _(not in config)_         | No                    | No                      | N/A (Private)             |
...
const res = await fetch(
  `/api/public-data/read/event_types?owner_id=${adminUserId}`,
);
...
The NCB MCP server is configured in `.mcp.json`:
...
- `set_rls_policy` - Set RLS policies for tables
- `get_rls_policies` - Fetch current RLS configuration
- `create_table` - Create tables in NCB
- `list_tables` - List all tables

Use the MCP server when you need to programmatically set policies for public tables.

---

## Completion Checklist

Data proxy setup is complete when:

- [ ] `lib/ncb-utils.ts` created with all utilities
- [ ] `app/api/data/[...path]/route.ts` created (authenticated)
- [ ] `app/api/public-data/[...path]/route.ts` created (public)
- [ ] `credentials: "include"` used in authenticated requests
- [ ] RLS policies set for public tables (via MCP or dashboard)
- [ ] Authenticated reads return only user's records
- [ ] Public reads work for tables with `public_read` policy
- [ ] Public creates work for tables with `public_write` policy
